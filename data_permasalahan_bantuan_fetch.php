...
$query .= "SELECT * FROM data_permasalahan INNER JOIN (user_details JOIN provinsi ON user_details.provinsi=provinsi.id_provinsi) ON data_permasalahan.user_id=user_details.user_id WHERE status='takTerjawab' AND";
...
function get_total_all_records($connect)
{
	...
	$statement = $connect->prepare("SELECT * FROM data_permasalahan WHERE status='takTerjawab'");
	$statement->execute();
