...

{
	...
	$sub_array = array();
	$sub_array[] = get_permasalahan_id($connect, $row['solusi_id']);
...
function get_total_all_records($connect)
{
	...
	$statement = $connect->prepare("SELECT * FROM data_solusi");
	$statement->execute();
...
function get_permasalahan_id($connect, $solusi_id)
{
	$query="
	SELECT * FROM data_solusi INNER JOIN data_permasalahan ON data_solusi.permasalahan_id=data_permasalahan.permasalahan_id WHERE solusi_id ='".$solusi_id."'
	";
...
function get_waktu_permasalahan($connect, $solusi_id)
{
	...
	foreach($result as $row)
	{
		return date("F j, Y H:i:s",strtotime($row['waktu_permasalahan']));
