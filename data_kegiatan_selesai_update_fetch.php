...
{
	$query .= 'WHERE kegiatan_selesai_id LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['waktu_kegiatan_selesai'];
	...
	$sub_array[] = '<button type="button" name="update" id="'.$row["kegiatan_dimulai_id"].'" class="btn btn-warning btn-xs update">Update</button>';
...
function get_total_all_records($connect)
{
	...
	$statement = $connect->prepare("SELECT * FROM data_kegiatan_selesai");
	$statement->execute();
...
function get_waktu_kegiatan($connect, $kegiatan_selesai_id)
{
	...
	foreach($result as $row)
	{
		return $row['waktu_kegiatan_dimulai'];
	}
}
