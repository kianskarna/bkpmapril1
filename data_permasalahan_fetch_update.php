...
$query .= "SELECT * FROM data_permasalahan INNER JOIN (user_details JOIN provinsi ON user_details.provinsi=provinsi.id_provinsi) ON data_permasalahan.user_id=user_details.user_id ";

if(isset($_POST["search"]["value"]))
{
	$query .= 'WHERE permasalahan_id LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['waktu_permasalahan'];
	...
	if($_SESSION["type"] == 'master')
		{
			$sub_array[] = '<button type="button" name="update" id="'.$row["permasalahan_id"].'" class="btn btn-warning btn-xs update">Update</button>';
			$sub_array[] = '<button type="button" name="delete" id="'.$row["permasalahan_id"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["status"].'">Delete</button>';
		}
...
function get_total_all_records($connect)
{
	...
	$statement = $connect->prepare("SELECT * FROM data_permasalahan INNER JOIN (user_details JOIN provinsi ON user_details.provinsi=provinsi.id_provinsi) ON data_permasalahan.user_id=user_details.user_id");
	$statement->execute();
