...
$query .= "
SELECT * FROM user_details INNER JOIN provinsi ON user_details.provinsi=provinsi.id_provinsi
WHERE user_type = 'helpdesk' AND 
";

if(isset($_POST["search"]["value"]))
{
	$query .= '(user_email LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR no_telepon LIKE "%'.$_POST["search"]["value"].'%" ';
...
function get_total_all_records($connect)
{
	...
	$statement = $connect->prepare("SELECT * FROM user_details INNER JOIN provinsi ON user_details.provinsi=provinsi.id_provinsi WHERE user_type='helpdesk'");
	$statement->execute();
