...
$query .= "SELECT * FROM data_kegiatan_dimulai INNER JOIN user_details ON data_kegiatan_dimulai.user_id=user_details.user_id JOIN provinsi ON user_details.provinsi=provinsi.id_provinsi ";
if(isset($_POST["search"]["value"]))
{
	$query .= 'WHERE kegiatan_dimulai_id LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['waktu_kegiatan_dimulai'];
...
function get_total_all_records($connect)
{
	...
	$statement = $connect->prepare("SELECT * FROM data_kegiatan_dimulai");
	$statement->execute();
